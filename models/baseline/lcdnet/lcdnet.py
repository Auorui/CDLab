...
from models.baseline.lcdnet.backbone import mobilenet_v2
...
class FFM(nn.Module):
    ...
    def forward(self,x1,x2):
        x1 = self.conv1(x1)   # 对 T1 特征做 1×1 卷积
        x1 = self.relu(x1)    # ReLU 激活
        x2 = self.conv1(x2)   # 对 T2 特征做同一个 1×1 卷积
        x_x = x1 * x2         # 元素级相乘
        x_x = self.relu(x_x)  # ReLU 激活
        x_x = x_x + x2        # 残差加法
        x_x = x_x * x1        # 再次元素级相乘
        x_x = self.relu(x_x)  # 最终激活
        return x_x
...
class LCD_Net(nn.Module):
    def __init__(self, num_classes=2, pretrained=True):
        ...
        mob = mobilenet_v2(pretrained=pretrained)
        ...
        self.decoder = nn.Sequential(
            SqueezeDoubleConvOld(472, 64),
            nn.Conv2d(64, num_classes, 1)
        )

        self.decoder_4 = SqueezeDoubleConvOld(64 * 2 + num_classes, 64)
        self.decoder_3 = SqueezeDoubleConvOld(64 * 3 + num_classes, 64)
        self.decoder_2 = SqueezeDoubleConvOld(64 * 3 + num_classes, 64)
        self.decoder_1 = SqueezeDoubleConvOld(64 * 3 + num_classes, 64)
        self.decoder_final = nn.Sequential(
            SqueezeDoubleConvOld(64, 64),
            nn.Conv2d(64, num_classes, 1)
        )
    ...
    def forward(self,A,B):
        ...
        # 输出的通道数统一压缩到 64, 似乎做的是通道对齐 + 特征增强, 论文图中未给出
        ...
        feature_fuse = layer_ss                  # [N, 472, H/2, W/2]
        change_map = self.decoder(feature_fuse)  # [N, 2, H/2, W/2]
        ...
        layer4_1 = torch.cat([layer4_1, change_map1], dim=1)   # [N, 64*2 + 2, H/2, W/2] = [N, 130, H/2, W/2]
        layer4_1 = self.decoder_4(layer4_1)                    # → [N, 64, H/2, W/2]
        layer3_1 = torch.cat([layer4_1, layer3_1, change_map1], dim=1)  # [N, 64 + 64*2 + 2, H/2, W/2] = [N, 194, H/2, W/2]
        layer3_1 = self.decoder_3(layer3_1)                             # → [N, 64, H/2, W/2]
