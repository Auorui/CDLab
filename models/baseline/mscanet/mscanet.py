...
from models.baseline.mscanet.backbone import build_resnet_backbone
from models.baseline.mscanet.modules import TransformerDecoder, Transformer
...
class token_decoder(nn.Module):
    ...
    def forward(self, x, m):
        ...
        if h != self.size or w != self.size:
            ...
            pos_embed = self.pos_embedding_decoder
        x = x + pos_embed
...
class MSCANet(nn.Module):
    def __init__(self, backbone='resnet18', output_stride=16, img_size=512, chan_num=32, num_classes=2):
        super(MSCANet, self).__init__()

        self.backbone = build_resnet_backbone(backbone, output_stride)
        ...
        self.classifier1 = Classifier(n_class=num_classes)
        self.classifier2 = Classifier(n_class=num_classes)
        self.classifier3 = Classifier(n_class=num_classes)
...
if __name__=="__main__":
    ...
    with torch.no_grad():
        out, out8, out16 = model(img1, img2)
    print("Output shape (final):", out.shape)
    print("Output shape (scale8):", out8.shape)
    print("Output shape (scale16):", out16.shape)
