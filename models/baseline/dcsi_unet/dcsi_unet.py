...
class DCSI_UNet(nn.Module):
    def __init__(self, num_classes=2):
        ...
        self.Pre_conv1 = nn.Sequential(Conv(filters[0], 16, 3, bn=True), Conv(16, self.num_classes, 3, bn=False, relu=False))
        self.Pre_conv2 = nn.Sequential(Conv(filters[0], 16, 3, bn=True), Conv(16, self.num_classes, 3, bn=False, relu=False))
        self.Pre_conv3 = nn.Sequential(Conv(filters[0], 16, 3, bn=True), Conv(16, self.num_classes, 3, bn=False, relu=False))
    ...
    def forward(self, Img1, Img2):
        ...
        Fc1 = self.CGIM1(F1_1, F2_1)
        Fc2 = self.CGIM2(F1_2, F2_2)
        Fc3 = self.CGIM3(F1_3, F2_3)
        Fc4 = self.CGIM4(F1_4, F2_4)

        Fs1 = self.SGAM1(F1_1, F2_1)
        Fs2 = self.SGAM2(F1_2, F2_2)
        Fs3 = self.SGAM3(F1_3, F2_3)
        Fs4 = self.SGAM4(F1_4, F2_4)
        ...
        # IAM stage / IAM 级联处理
        Fg_4 = torch.cat([Fc4, Fs4], dim=1)
        Fg_3 = torch.cat([Fc3, Fs3], dim=1)
        Fg_2 = torch.cat([Fc2, Fs2], dim=1)
        Fg_1 = torch.cat([Fc1, Fs1], dim=1)
        ...
        # return P1, P2, P3
        return P3, P2, P1    # 推理的代码默认是使用第一个，因此要改变一下顺序
