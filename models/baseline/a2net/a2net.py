...
from models.baseline.a2net.backbone import mobilenet_v2
...
class SupervisedAttentionModule(nn.Module):
    def __init__(self, mid_d, num_classes):
        super(SupervisedAttentionModule, self).__init__()
        self.mid_d = mid_d
        # fusion
        self.cls = nn.Conv2d(self.mid_d, num_classes, kernel_size=1)
        self.conv_context = nn.Sequential(
            nn.Conv2d(num_classes * 2, self.mid_d, kernel_size=1),
            nn.BatchNorm2d(self.mid_d),
            nn.ReLU(inplace=True)
        )
...
class Decoder(nn.Module):
    def __init__(self, mid_d=320, output_nc=2):
        super(Decoder, self).__init__()
        self.mid_d = mid_d
        # fusion
        self.sam_p5 = SupervisedAttentionModule(self.mid_d, output_nc)
        self.sam_p4 = SupervisedAttentionModule(self.mid_d, output_nc)
        self.sam_p3 = SupervisedAttentionModule(self.mid_d, output_nc)
        ...
        self.cls = nn.Conv2d(self.mid_d, output_nc, kernel_size=1)
...
class A2Net(nn.Module):
    def __init__(self, num_classes=2, pretrained=True):
        ...
        self.decoder = Decoder(self.en_d * 2, num_classes)
