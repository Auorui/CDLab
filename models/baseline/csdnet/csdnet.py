...
from models.baseline.csdnet.fpn import FPN
from models.baseline.csdnet.decode_block import ResDecodeBlock
from models.baseline.csdnet.StCoBlock import StyleStrip, StyleContextModule, StyleContextModuleLite
from models.baseline.csdnet.ccr_block import ContextualContentRefiner
from models.baseline.csdnet.backbone import build_backbone
from models.baseline.csdnet.exchange import FeatureExchanger, ExchangeType


class CSDNet(nn.Module):
    def __init__(self, model_name='hrnet_w64', num_classes=2):
        ...
        self.fpn = FPN(in_channels=FPN_DICT['in_channels'], out_channels=FPN_DICT['out_channels'], num_outs=FPN_DICT['num_outs'])
        ...
        self.conv_seg = nn.Conv2d(FPN_DICT['out_channels'], num_classes, kernel_size=1)
    ...
    def forward(self, xA, xB):
        ...
        if self.training:
            return [outA, outB]  # 训练：返回列表，分别计算 loss
        else:
            return (outA + outB) / 2.0  # 推理：返回平均值，单个 tensor
