...
class ELGCNet(nn.Module):
    def __init__(self, input_nc=3, num_classes=2, depths=[3, 3, 4, 3], heads=[4, 4, 4, 4],
        ...
        # shared encoder
        self.enc = Encoder(patch_size=7, in_chans=input_nc, num_classes=num_classes, embed_dims=self.embed_dims,
                           heads=heads, mlp_ratios=[4, 4, 4, 4], drop_path_rate=self.drop_path_rate, depths=self.depths)

        # decoder
        self.dec = Decoder(in_channels=self.embed_dims, embedding_dim=self.embedding_dim, output_nc=num_classes,
                           align_corners=False)
