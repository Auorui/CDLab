...
from models.baseline.stnet.backbone import build_resnet_backbone
...
class STNet(nn.Module):
    def __init__(self, backbone='resnet18', num_classes=2, channel_list=[64, 128, 256, 512],
                 transform_feat=128, layer_num=4, pretrained=True):
        super(STNet, self).__init__()
        self.backbone = build_resnet_backbone(backbone, pretrained)
        self.layer_num = layer_num
        ...
        self.lightdecoder = LightDecoder(transform_feat, num_classes, layer_num)
    
    def forward(self, img1, img2):
        xA1, xA2, xA3, xA4 = self.backbone(img1)
        xB1, xB2, xB3, xB4 = self.backbone(img2)
